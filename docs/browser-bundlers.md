...
Remote reads use the same 30-second default deadline and 100 MB body budget as
Node. Pass `fetchTimeoutMs`, `fetchMaxBytes`, or `signal` to `openPdf` and
`extractPdf` to customize or cancel a request without relying on
